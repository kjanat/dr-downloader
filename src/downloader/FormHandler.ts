...
import type { Platform, RegistrationData } from "../config/types.ts";
...
export class FormHandler {
  ...
  async fillRegistrationForm(data: RegistrationData): Promise<void> {
    ...
    await this.ensureChecked(SELECTORS.policy);
  }

  async clickPlatformDownload(platform: Platform): Promise<void> {
    const buttonText = SELECTORS.platformButtons[platform];
  ...
  private async selectCountryAndState(data: RegistrationData): Promise<void> {
    ...
    if (data.state) {
      await this.selectByValueOrText(SELECTORS.state, data.state, data.state);
    }
  }

  private async ensureChecked(selector: string): Promise<void> {
    try {
      await this.page.click(selector, { delay: 10 });
